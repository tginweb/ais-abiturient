...
import {EduEpguMessageService} from "./service";
...
export class EduEpguMessageModel extends BaseModel {
    ...
    @prop({ref: () => AbitOrderModel, localField: "response.GuidEntrant", foreignField: "epgu.guid", justOne: true})
    entrantOrder?: Ref<AbitOrderModel>

    @prop({ref: () => AbitAppGroupModel, localField: "response.GuidApplication", foreignField: "epguGuid", justOne: true})
    ...
    async getToken(refetch = false) {
        if (!this.request.token || refetch || 1) {
            this.request.token = await this.context.epguApiService.tokenGenerate(this.request.header['toJSON'](), await this.getPayloadXml())
        }
    ...
    async getInfoToken(refetch = false) {
        ...
        if (!this.infoToken || refetch) {
            this.infoToken = await this.context.epguApi.tokenGenerate({
                Action: 'GetMessage',
                IdJwt: this.idJwt
            }, null)
        }
    ...
    async onSend() {
        ...
        if (asyncMode) {
            this.context.epguApiService.tokenNew(await this.getToken(), {...this.getMeta(), async: true})
        } else {
            res = await this.context.epguApiService.tokenNew(await this.getToken(), this.getMeta())
        }
    ...
    async fetchResult(refetch = false) {
        ...
        if (!this.response.payloadXml) {
            ...
            const res = await this.context.epguApiService.getTokenInfo(this.queue, this.idJwt)

            if (res && res.Token) {

                token = await this.context.epguApiService.responseTokenDecode(res.Token)

                token.payload = await this.context.epguApiService.responsePayloadToJson(token.payloadXml)
            } else {
                console.log(res)
            }
    ...
    fillRequestHeader(header: DocRequestHeader = {}) {
        this.request.header.Entity = header.Entity || this.epguEntityType
        this.request.header.Action = header.Action || this.epguAction
    }

    fillEntityArgs(entity) {
        this.args.entityId = entity._id
        this.args.entityNid = entity.getNid && entity.getNid()
        this.args.entityTitle = entity.getEpguTitle && entity.getEpguTitle()
    }
    ...
    convertEpguDate(date, format = 'DD.MM.YYYY') {
