...
export class EduFisService {
    ...
    async getDirectionByOkso(okso) {
        const filepath = path.join(process.cwd(), 'db/fis/directions.json')

        const items = JSON.parse(await readFile(filepath, 'utf8'));

        const itemsByOkso = items.reduce((map, item) => {
            map[item.NewCode] = item
            return map
        }, {})

        return itemsByOkso[okso]
    }
}
