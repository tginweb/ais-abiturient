...
export class EduFisMessageModel extends BaseModel {
    ...
    @prop({ref: () => AbitOrderModel, localField: "response.GuidEntrant", foreignField: "fis.guid", justOne: true})
    entrantOrder?: Ref<AbitOrderModel>

    @prop({
        ref: () => AbitAppGroupModel,
        localField: "response.GuidApplication",
        foreignField: "fisGuid",
        justOne: true
    })
    ...
    async getToken(refetch = false) {
        if (!this.request.token || refetch || 1) {
            this.request.token = await this.context.fisApiService.tokenGenerate(this.request.header['toJSON'](), await this.getPayloadXml())
        }
    ...
    async getInfoToken(refetch = false) {
        ...
        if (!this.infoToken || refetch) {
            this.infoToken = await this.context.fisApi.tokenGenerate({
                Action: 'GetMessage',
                IdJwt: this.idJwt
            }, null)
        }
    ...
    async onSend() {
        ...
        if (asyncMode) {
            this.context.fisApiService.tokenNew(await this.getToken(), {...this.getMeta(), async: true})
        } else {
            res = await this.context.fisApiService.tokenNew(await this.getToken(), this.getMeta())
        }
    ...
    async fetchResult(refetch = false) {
        ...
        if (!this.response.payloadXml) {
            ...
            const res = await this.context.fisApiService.getTokenInfo(this.queue, this.idJwt)

            if (res && res.Token) {

                token = await this.context.fisApiService.responseTokenDecode(res.Token)

                token.payload = await this.context.fisApiService.responsePayloadToJson(token.payloadXml)
            } else {
                console.log(res)
            }
    ...
    async getPayloadXml(refetch = false) {
        if (!this.request.payloadXml || refetch) {
            ...
            const fileDir = path.join(process.cwd(), 'fis-packet')
    ...
    fillRequestHeader(header: DocRequestHeader = {}) {
        this.request.header.Entity = header.Entity || this.fisEntityType
        this.request.header.Action = header.Action || this.fisAction
    }

    fillEntityArgs(entity) {
        this.args.entityId = entity._id
        this.args.entityNid = entity.getNid && entity.getNid()
        this.args.entityTitle = entity.getFisTitle && entity.getFisTitle()
    }
    ...
    convertFisDate(date, format = 'DD.MM.YYYY') {
