<template>
  ...
  <div>
    ...
    <div class="q-mb-sm text-grey-7 s-font-sm" v-if="restrictionsShowComp">
    ...
    <div class="">
      ...
      <div v-if="valueData.length" class="q-mt-sm q-mb-md" style="vertical-align: middle">
        ...
        <div v-for="file of valueData" class="c-file flex no-wrap items-center q-my-sm">
...
export default {
  ...
  computed: {
    ...
    isEmpty() {
      return !this.valueData.length || this.valueData.find(item => !item)
    },

    restrictionsShowComp() {
      return this.restrictionsShow && this.isEmpty
    },
    ...
    compRestrictionsLabel() {
      ...
      if (this.accept)
        parts.push('формат: ' + this.accept.split(', ').map(item => item.replace(/\./, '').toUpperCase()).join(', '))

      if (this.maxFileSize)
        parts.push('размер <= ' + this.maxFileSizeText)
