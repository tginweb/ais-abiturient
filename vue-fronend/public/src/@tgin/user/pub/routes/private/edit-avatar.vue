...
export default {
  ...
  computed: {
    ...
    avatarsOptions() {
      return this.$store.state.user.app.avatarElements
          .filter(item => !!item.LIST_IMAGE)
          .map(item => ({
            value: item.ID,
            image: 'img:' + item.LIST_IMAGE.SRC,
            imageSrc: item.LIST_IMAGE.SRC
          }))
    },
    ...
    onSubmit() {
      this.$refs.formAvatar.validate().then(async (success) => {
        ...
        if (success) {
          ...
          try {
            const {data} = await this.$api.post('/user/avatar/update', bodyFormData)
