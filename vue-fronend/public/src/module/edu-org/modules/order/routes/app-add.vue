              ...
              <div>
                ...
                <div class="">
                  ...
                  <div class="q-mr-md text-weight-bold text-primary1">
                    {{ prop.node.abbr }} {{ prop.node.direct_name }} {{ prop.node.eng ? ', англоязчная программа' : '' }}
                  </div>
                ...
                <div class="text-grey-7  q-mt-xs s-font-sm flex q-gutter-x-md" style="width1: 150px;">
                  ...
                  <div v-if="prop.node.lgotQuota > 0">Особое право: {{ prop.node.lgotQuota }}</div>
                  <div v-if="prop.node.specQuota > 0">Отдельная квота: {{ prop.node.specQuota }}</div>
...
export default {
  ...
  methods: {
    admissionSpecs(model) {
      ...
      return this.$store.getters['edu_order/userOrderAvailableAdmissions'].filter((program) => {
  ...
  computed: {
    ...
    bindApplicationOsnova() {
      let res = {
        options: []
      }

      if (this.selectedNode.places.budget > 0) {
        res.options.push({label: 'бюджетная', value: 'budget'})
      }
      if (this.selectedNode.places.target > 0) {
        res.options.push({label: 'целевая', value: 'target'})
      }
      if (this.selectedNode.places.commerce > 0) {
        res.options.push({label: 'коммерческая', value: 'commerce'})
      }

      return res
    },

    bindApplicationPriemCat() {
      let res = {
        options: []
      }

      if (this.selectedNode.places.quota > 0) {
        res.options.push({label: 'на общих основаниях', value: 'common'})
        res.options.push({label: 'особое право', value: 'common'})
      }
    ...
    contextAdmissions() {
      return this.$store.getters['edu_order/userOrderAvailableAdmissions'].filter(item => item.budgCount > 0 || item.comercCount > 0).map(item => {
        return {
          ...item,
    ...
    eduProgramsNodes() {
      ...
      this.contextAdmissionsFiltered.forEach((admission) => {
        ...
        if (['ИИКб','ЭПАб','ИЭб','МДБб','ФНб','ЖКб','ЛБКб','ТМб'].indexOf(admission.abbr)>-1) {
          admission.eng = true
        }
